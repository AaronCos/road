        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-body ">
                    <form class="layui-form layui-col-space5">
                        <div class="layui-inline layui-show-xs-block">
                            <input type="text" name="title" placeholder="请输入角色名称" autocomplete="on"
                                   class="layui-input">
                        </div>
                        <div class="layui-inline layui-show-xs-block">
                            <button class="layui-btn" lay-submit lay-filter="role_sreach_filter"><i class="layui-icon">&#xe615;</i>
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="layui-card-body layui-table-body layui-table-main">
                    <table id="role-table" class="layui-table layui-form" lay-filter="role-table-filter">
...
<script type="text/html" id="barManual">
    <button class="layui-btn layui-btn-danger" lay-event="delrole"><i class="layui-icon"></i>删除
    </button>
    <button class="layui-btn" onclick="xadmin.open('新增招聘','${path}/role/showadd.do',500,300)"><i
...
<script>
    ...
    layui.use(['laydate', 'form', 'table'], function () {
        ...
        table.render({
            ...
            elem: '#role-table',
            //height:400,
            url: '${path}/role/role-data.do',
            method: 'get',
            page: true,
            cols: [[
                {field: 'iid', type: 'checkbox', width: 80},
                {field: 'name', title: '角色名称', align: 'center'},
                {field: 'spec', title: '角色描述', align: 'center'},
                {field: 'pinYin', title: '拼音', align: 'center'},
                {field: 'type', title: '角色类型', align: 'center'},
                {field: 'isDefault', title: '是否默认角色', templet: '#is-default', align: 'center'},
            ]],
            limits: [10, 20, 30]
        });
        //支持全局搜索
        form.on('submit(role_sreach_filter)', function (obj) {
        ...
        table.on('tool(role-table-filter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                layer.open({
                    ...
                    content: ["${path}/role/showedit.do?iid=" + data.iid, "yes"],
        ...
        form.on('switch(pageshow-filter)', function(obj){
            ...
            $.ajax({
                url :  "${path}/role/changepageshow.do",
                type : "POST",
                datatype : "json",
                async : true,
                data : {
