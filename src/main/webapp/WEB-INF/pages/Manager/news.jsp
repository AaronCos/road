				...
				<div class="manage">
					<table class="list">
						...
						<f:forEach items="${requestScope.news}" var="n">
							<tr>
								...
								<td class="w1 c">
									<a href="DoBackGetNewsUpdate?newsId=${n.id }">修改</a>
									<a href="javascript:newsDelete(${n.id });">删除</a>
								</td>
							</tr>
						</f:forEach>
					</table>
					<div class="pager">
						...
						<f:if test="${requestScope.pageIndex >1}">
							<a href="DoBackNewsList?pageIndex=${requestScope.pageIndex -1}">上一页</a>
						</f:if>
						<f:if test="${requestScope.pageIndex  == 1}">
							上一页
						</f:if>
						<f:if test="${requestScope.pageIndex <requestScope.pageCount}">
							<a href="DoBackNewsList?pageIndex=${requestScope.pageIndex +1}">下一页</a>
						</f:if>
						<f:if test="${requestScope.pageIndex  == requestScope.pageCount}">
							下一页
						</f:if>
						<a href="DoBackNewsList?pageIndex=1">首页</a>
						<a href="DoBackNewsList?pageIndex=${pageCount}">末页</a>
